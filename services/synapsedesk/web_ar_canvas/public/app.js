...
let positions=new Map(),placed=new Set(),tracking=null,lastEvent=0,lastPinch=false,drag=null,hover=null,wireMode=false,wireSource=null,selectedId=null,allNodes=[];
...
let saveTimer=null;
function persistPositions(){if(DISPLAY)return;try{localStorage.setItem('synapsedesk.positions',JSON.stringify(Object.fromEntries(positions)));}catch{}clearTimeout(saveTimer);saveTimer=setTimeout(()=>{if(!token)return;fetch('/api/positions',{method:'POST',headers:{'Content-Type':'application/json','X-Synapse-Token':token},body:JSON.stringify({positions:Object.fromEntries(positions)})}).catch(()=>{});},800);}
...
function relayout(){for(const id of [...positions.keys()])if(!placed.has(id))positions.delete(id);refreshGraph();}
...
function toggleWire(){if(DISPLAY)return;wireMode=!wireMode;wireSource=null;release();$('wire')?.classList.toggle('active',wireMode);set('selection','textContent',wireMode?'Pinch source, release, then pinch destination. Proposed connections are saved.':'Pinch to pick up a node. Release to place it.');}
on('analyze','onsubmit',async e=>{e.preventDefault();try{await api('/api/analyze',{source:$('source').value});}catch(error){notify(error.message);}});
...
on('projector','onclick',toggleProjection);
...
canvas.onpointermove=e=>{if(pointerDown&&drag){const q=positions.get(drag);if(!q)return;q.x=Math.max(.06,Math.min(.94,e.clientX/width));q.y=Math.max(.1,Math.min(.9,e.clientY/height));placed.add(drag);}};
...
function draw(time){ctx.clearRect(0,0,width,height);ctx.fillStyle='#020608';ctx.fillRect(0,0,width,height);
 ...
 hover=hit(cursor);
 if(!pointerDown&&!calibration){const pinch=enabled&&tracking.pinch;if(pinch&&!lastPinch){if(wireMode)connectNode(hover);else drag=hover;}if(!pinch&&drag){persistPositions();drag=null;}if(drag&&cursor){const p=positions.get(drag);if(p){p.x=Math.max(.06,Math.min(.94,cursor[0]/width));p.y=Math.max(.1,Math.min(.9,cursor[1]/height));placed.add(drag);}}lastPinch=!!pinch;}
...
async function syncPositions(force=false){try{const p=await fetch('/api/positions');if(!p.ok)return;const pj=await p.json();for(const [k,v] of Object.entries(pj.positions||{})){if(force||!positions.has(k))positions.set(k,v);placed.add(k);}}catch{}}

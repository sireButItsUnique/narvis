 ...
 */
// Elastic Grab evaluates this once per vertex of the WHOLE part, so on a 150k-triangle model that
// is 75,000 calls per dab; a fresh array each time cost 17 ms of the 24 ms the loop took. The
// arithmetic is unchanged - these are scratch, written in full before they are read.
const _weights = [0, 0, 0];
const _triWeights = [0, 0, 0];
...
export function kelvinletGrabTriscale(out, p, position, location, delta) {
  ...
  const w = _triWeights;
  w[0] = 1; w[1] = -((s2 - s0) / (s2 - s1)); w[2] = (s1 - s0) / (s2 - s1);
